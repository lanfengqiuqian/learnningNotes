...
#### 微信小程序 webview 清除缓存、微信公众号 h5 清除缓存、页面白屏、空白、不刷新问题
...
2. 核心区别
   ...
   | 维度       | 微信公众号                       | 小程序                             |
   | ---------- | -------------------------------- | ---------------------------------- |
   | 触发条件   | 用户需要关注公众号即可           | 用户需要在小程序内授权弹窗同意     |
   | 推送形式   | 群发消息，模板消息，被动回复     | 订阅消息、服务通知                 |
   | 触达率     | 服务号消息直接显示在聊天列表     | 类似系统通知，打开率高             |
   | 内容灵活性 | 群发内容自由编辑，模板消息固定   | 订阅消息克自定义，服务通知格式固定 |
   | 使用场景   | 内容运营（新闻、活动）、服务提醒 | 即时服务通知（订单、物流）         |

#### 微信开发者工具如何调试微信扫小程序码进入的场景

1. 在`添加编译模式`中选择】`添加方式`为`解析二维码`
2. 然后上传二维码
3. 然后会把二维码自动识别出参数和路径
...
在`view`下面有`表单输入元素`的时候会有穿透问题，即使上层有元素的 click 也不会触发
