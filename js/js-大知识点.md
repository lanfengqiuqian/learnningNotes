...
1. 浏览器进程模型
    ...
    3. 浏览器有哪些进程和线程
        ...
        2. 网络进程：负责加载网络资源。
        3. 渲染进程：渲染进程启动之后，会开启一个`渲染主线程`，主线程负责执行html、css、js代码。默认情况下，浏览器会为每个标签页开启一个新的渲染进程，以保证不同的标签页之间不相互影响。
    ...
    4. 渲染主线程是如何工作的
        ...
        比如：我正在执行一个js函数，执行到一半的时候用户点击了按钮，我该立即去执行点击事件的处理函数吗？
...
5. 队列类型
    ...
    3. 微队列，用于存放需要最快执行的任务，如promise，优先级【最高】
