...
2. get依赖收集阶段
    ...
    通过解析`template`，替换真实`data属性`，来触发`get`，然后通过`stack`方法，通过`proxy`对象和`key`形成对应的`deps`,将负责渲染的`effect`存入`deps`
...
vue3：销毁的生命周期改为了beforeUnmount、unmounted

特殊钩子：activated、deactivated（keep-alive相关）

常用场景：created发起ajax请求、mounted操作dom、beforeUpdate获取更新前滚动位置、beforeDestroy清除定时器、取消事件监听、清理订阅

### 父子组件加载顺序

父组件：beforeCreate、created、beforeMount
子组件：beforeCreate、created、beforeMount、mounted、beforeDestroy、destroyed
子组件：mounted、beforeDestroy、destroyed
...
1. 项目准备
   1. 确保项目已构建生成静态资源，如执行`npm run build`
   2. 创建`.dockerignore`文件排除`node_modules`等无关文件
2. 编写`Dockerfile`构建方案
3. 配置`nginx`
4. 构建镜像
5. 运行容器
6. 验证部署
