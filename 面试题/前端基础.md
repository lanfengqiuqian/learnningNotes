...
是客户端和服务器建立连接之前，必须完成的3次报文交互，确认双方收发能力都正常

1. SYN：客户端像服务器发送一个syn报文，并选择一个自己的初始序列号seq
2. SYN+ACK：服务器收到客户端的SYN后，如果同意连接，会回复一个SYN+ACK报文，包含ack这个序列号的确认号
3. ACK：客户端收到服务器的syn+ack后，会再次向服务器发送一个ack报文

为什么不是两次或者四次，两次无法确认通信是否正常，四次有点多余
...
1. FIN：客户端发送fin报文
2. ACK：服务端收到请求，回复ACK报文确认
3. FIN：服务端处理完数据之后，向客户端发送FIN报文
4. ACK：客户端收到服务端报文之后，回复服务端ACK报文
...
1. dns解析url（将域名翻译为ip地址）
    ...
    1. 先会判断输入的是一个搜索关键词，还是合法的url
    2. 如果本地缓存了这个资源，则直接返回缓存了的资源，如果没有缓存，则会返回DNS解析域名
    3. 从浏览器dns缓存中查找是否有缓存（也就是之前的访问记录），如果有则返回ip
    4. 如果没有的话，查询本地host文件
    5. 如果还没有的话，递归查询上层dns服务器，直到能拿到结果
    6. 浏览器拿到ip之后，先要和服务器建立tcp连接

2. 浏览器和服务器进行交互

    1. 浏览器利用TCP协议进行3次握手和服务器建立连接
       1. 客户端发送`SYN`包给服务器
       2. 服务器收到客户端的包，返回`SYN/ACK`的响应包
       3. 客户端收到服务器的包，发送`ACK`的包
    2. 浏览器根据解析到的ip地址和端口发起http的请求
       1. 请求分为三部分：请求行（请求方法、http协议，版本）
       2. 请求头
       3. 请求正文（和请求头之间是一个空行，这代表请求头已经结束）
    3. 服务器从固定端口收到tcp报文，对http协议进行解析，按照报文格式进一步封装成`http request`对象
       1. 一般大型的网站会使用nginx进行反向代理
       2. 大概意思就是访问量过大，一台服务器负担不起，先将请求转发到一个代理服务器
       3. 然后这个代理服务器根据负载判断转发给哪个服务器，达到负载均衡
       4. 还有一个好处就是，如果某台服务器挂了，则不影响正常访问
    4. 然后服务器重新返回一个http响应，同样分为响应行、响应头、响应正文
    5. 浏览器接收到数据之后，如果资源进行了压缩，需要先进行解压

3. 浏览器渲染页面的过程

    1. html经过html解析器生成dom树
    2. css经过css解析器生成cssom树
    3. 将dom树和cssom规则树构造成render树
    4. 浏览器根据dom树和cssom进行布局和绘制，会触发回流和重绘
    5. 构建图层树，显示页面
    6. 客户端触发四次挥手关闭连接
       1. 客户端发送`FIN`包给服务器，告知服务器自己不接收数据了
       2. 服务器收到报文之后，发送`ACK`包给客户端，告诉客户端自己不发送数据了
       3. 服务端发送`FIN/ACK`包给客户端，表示自己不接收数据了
       4. 客户端发送`ACK`包给服务端，表示自己不发送数据了
...
1. prototype是函数独有的，而__proto__是每个对象都会有的
2. prototype的作用是保存所有实例的公共属性和方法；__proto__作用是当访问一个对象的属性时，如果内部没有该属性，就会在他的__proto__属性所指的那个父对象去找，父对象没有，再去父对象的父对象找，直到找到null位置，即原型链
3. p.__proto__ = P.prototype;
4. prototype还有一个属性constract，指向该对象的构造函数本身。P.prototype.coustract = P
...
1. 标准问题
    ...
    在4层之后的计时器，会自动加上`4ms`的延迟，这是js标准决定的

2. 事件循环

    如果计时器的时间到了，但是任务队列还没有执行完毕，那么也无法开始执行


### js和css解析是否会影响渲染

1. `css`不会阻塞`dom`解析，但是会阻塞`dom`渲染，因为`cssom tree`和`dom tree`组成`render tree`
2. `js`会阻塞`dom的解析`，因为假如不阻塞的话，可能`dom`解析完之后是无效的，内部的`dom tree`被js清空了
3. `css`会阻塞`js`的执行，如果`js`脚本内容是获取`css`样式属性，如果`js`想要获取到`css`最新的样式属性，只有等到`css`获取完成之后
...
| 慢                          | 快                                       |
| 重量级                      | 轻量级                                   |
| web浏览，文件传输，电子邮件 | 实时性要求很高：电视、电话、在线游戏、广播 |
...
这个语法，fn会直接执行，demo1和demo2会放到微队列中
```js
async function foo() {
