 ...
 * @LastEditors: Lq
 * @LastEditTime: 2022-08-08 13:26:22
...
readyState用来标志当前xhr对象处于什么状态

status表示http响应的状态


### ajax fetch axios等优缺点

|              | 介绍                                   | 优点                                                                                                | 缺点                                                             |
| ------------ | -------------------------------------- | --------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------- |
| ajax         | 是原生的js请求                         | 不需要引用任何库就能使用                                                                            | 使用繁琐，需要配置各种参数，兼容性不太好                         |
| jQuery的ajax | 本身是对于xhr 的封装，并处理了兼容问题 | 做了兼容处理，简化使用，增加了对于jsonp的支持                                                       | 如果多个请求会形成回调地狱，如果只是使用ajax而引入jQuery库不合理 |
| fetch        | 是原生js请求                           | 语法简单，更加语义化，基于Promise实现支持async/await，api丰富，脱离了xhr，是新的实现方式            | 只对于网络请求报错（对400、500都当做成功的请求，需要封装处理），没有办法原生监测请求的进度，xhr可以          |
| axios        | 对于xhr的封装，不过是promise的实现版本 | 可以在浏览器和node中使用，支持promise，自动转换json数据，可以拦截请求和响应，转换请求数据和响应数据 | 只支持现代浏览器                                                 |
...
1. 渐进式web应用
2. 本质上是一个网页
3. 加上`App Manifest`和`Service Worker`来实现pwa的安装和离线等功能
