...
10. 复制、拷贝、替身的区别
    ...
    1. 内容不同
       ...
       1. 复制：直接生成一个相同的文件（相当于 ctrl+c,ctrl+v）
       2. 拷贝：复制内容，但不生成文件（相当于 ctrl+c），到另外一个目录可以进行粘贴
...
13. mac 中.bash_profile 和 .zshrc 区别
    ...
    3. .zshrc ，source ~/.zshrc，永久生效；计算机每次启动自动执行 source ~/.zshrc

    4. 一般会在~/.zshrc 中添加 source ~/.bash_profile，以确保.bash_profile 中的修改永久生效。

14. 设置 zsh 终端不区分大小写路径提示

    背景：默认情况下，zsh 终端大小写不敏感，如`cd desktop`也能到桌面，但是按`tab`没有路径提示和补全
...
17. mac 安装 win10 虚拟机
    ...
    1. 使用 virtualbox（免费）
       ...
       1. virtualbox 安装包（实测 7.0.6 的版本还是没有安装上）
       ...
       2. win 系统镜像
          ...
          实测大坑，mac 的 m1 芯片需要使用 arm 架构的系统镜像才行
       ...
       3. 安装
          ...
          1. 先一直下一步安装好 virtualbox
          2. 新建虚拟机
          3. 选择下载好的镜像位置
          4. 内存 4g，内核 2cpu
          5. 选择创建虚拟硬盘，我选择的 100GB

       4. 设置虚拟机

          1. 工具 -> 全局设定 -> 语言 -> 简体中文
          2. win10 实例 -> 系统设置 -> 显示 -> Scale Factor 200%
          3. win10 实例 -> 系统设置 -> 存储 -> 如果提示没有盘片（则创建一个，选择之前的镜像）
          4. win10 实例 -> 系统设置 -> 常规 -> 共享粘贴板（双向）、拖放（双向）
          5. win10 实例 -> 系统设置 -> 存储 -> 控制器：SATA -> 勾选使用主机输入输出（I/O）缓存
          6. win10 实例 -> 系统设置 -> 存储 -> win10.vdi -> 勾选固态驱动器

       5. 设置主机和虚拟机间的共享文件夹，先在主机创建一个文件夹

          1. 在用户目录下的 VirtualBox VMs 目录下，和 win10 同级。
          2. 创建一个叫做 datashare 的目录
          3. win10 实例 -> 系统设置 -> 共享文件夹 -> 设置共享文件夹路径

       6. 双击启动虚拟机，进入 win10 装机步骤
    ...
    2. 使用 parallels desktop（需要付费，14 天试用）
       ...
       2. 安装之后提示没网
          ...
          按`win + e`打开我的电脑，会有一个 dvd 驱动器，双击安装他，然后重启就好啦
...
22. 安装 oh-my-zsh 之后出现 node、npm 等命令找不到
...
29. app已损坏，无法打开。你应该讲他移动到废纸篓

    <https://www.cnblogs.com/cnndevelop/p/18702837>

    1. 终端输入`xattr -cr `，注意后面有一个空格
    2. 将应用程序中无法打开的app拖入到终端中
    3. 然后命令就是`xattr -cr 程序路径`，直接回车
    4. 然后重新去打开程序就行了
...
30. 拷贝文件路径
    ...
    2. 右键
    3. 按住option
    4. 然后原本的【拷贝】就会变成拷贝路径了

31. 查看文件信息（不是预览文件）

    快捷键：cmd + i

32. 打开软件提示【xxx想访问其他App的数据】

    如果使用频率高的话，在`系统偏好设置` `隐私与安全性` `完全磁盘访问权限`

    加入这个程序，打开开关即可

33. app已损坏，无法打开，你应该推出磁盘映像

    <https://imacos.top/2023/06/08/1123-5/>

    1. 终端输入`sudo xattr -r -d com.apple.quarantine `(记得最后加个空格)
    2. 将app拖入终端窗口